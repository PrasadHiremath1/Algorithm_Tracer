...
export const dropFromQueue2 = (tile: TileType, queue: PriorityQueue<TileType>) => {
    // Filter out the tile you want to remove
    const filteredTiles = [];
    while (queue.length > 0) {
        const currentTile = queue.dequeue();
        if (!isEqual(currentTile, tile)) {
            filteredTiles.push(currentTile);
        }
    }

    // Re-add the remaining tiles to the queue
    filteredTiles.forEach((remainingTile) => queue.queue(remainingTile));
};